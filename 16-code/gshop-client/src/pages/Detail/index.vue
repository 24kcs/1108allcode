            ...
            <div class="chooseArea">
              ...
              <dl v-for="(attr,index) in spuSaleAttrList" :key="attr.id">
                ...
                <dd
                  ...
                  v-for="(spuAttr,index) in attr.spuSaleAttrValueList"
                  :key="spuAttr.id"
                  @click="setValue(spuAttr,attr.spuSaleAttrValueList)"
                >{{spuAttr.saleAttrValueName}}{{getTxt(attr)}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
...
export default {
  ...
  // 页面加载的生命周期回调
  mounted() {
    // 通过当前 路由信息对象 获取params参数中的数据
    const { skuId } = this.$route.params
    // 提交对应的action 并传入需要的参数
  ...
  methods: {
    ...
    setValue(spuAttr, spuSaleAttrValueList) {
      ...
      if (spuAttr.isChecked !== -1) {
        // 先把当前这个数组中的每个选项全部的变成0,意味着都没有选中-----重置
        spuSaleAttrValueList.forEach(val => (val.isChecked = 0))
        // 单独的设置当前这个选中的
        spuAttr.isChecked = '1'
      }
    },
    ...
    getTxt(attr) {
      ...
      // if(attr.saleAttrName==='选择颜色'){
      //   return '色'
      // }else{
      //   return ''
      // }
      // if (
      //   spuAttr.saleAttrValueName === '绿' ||
      //   spuAttr.saleAttrValueName === '白' ||
      //   spuAttr.saleAttrValueName === '黑'
      // ) {
      //   return '色'
      // } else {
      //   return ''
