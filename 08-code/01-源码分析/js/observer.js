...
Observer.prototype = {
    ...
    defineReactive: function(data, key, val) {
        ...
        Object.defineProperty(data, key, {
            ...
            set: function(newVal) {
                // 判断当前的要修改的属性的新值和旧值是否一致
                if (newVal === val) {
                    return;
                }
                // 说明新值和旧值不同,要进行修改
                val = newVal;
                // 新的值如果是对象则要遍历这个对象及里面的属性,创建对应的dep对象
                childObj = observe(newVal);
                // 通知订阅者:----->让当前这个属性对应的dep对象找到de对象内部subs数组中存储的watcher对象,进行数据的更新操作
                dep.notify();
            }
        });
    }
...
Dep.prototype = {
    ...
    // 遍历dep对象内部的subs数组中的watcher对象
    notify: function() {
        this.subs.forEach(function(sub) {
            // 调用watcher对象的update方法更新数据
            sub.update();
        });
    }
};
